...
<div class = "menuContentBox">
   <!--Contenitore per il menu con i relativi link per le altre pagine del sito-->
    ...
    ?>
		<form name = 'modify' action = 'saveModOperator.php?username=<?php echo $_GET['username']?>' method = 'POST'>
    	<label>Nome*:</label><br/>
   		<input name = 'nome' value = '<?php echo $row['nome']?>'/><br/><br/>
    	<label>Cognome*:</label><br/>
    	<input name = 'cognome' value = '<?php echo $row['cognome'] ?>'/><br/><br/>
		<label>CF:</label><br/>
    	<input name = 'cf' value = '<?php echo $row['cf'] ?>'/><br/><br/>
        <label>Data nascita*:</label><br/>
    	<input name = 'dataN' value = '<?php echo $row['dataNascita'] ?>' /><br/><br/>
		<label>Città:</label><br/>
    	<input name = 'citta' value = '<?php echo $row['citta'] ?>'/><br/><br/>
		<label>Telefono:</label><br/>
    	<input name = 'telefono' value = '<?php echo $row['telefono']?>' /><br/><br/>
		<tr>
		<label>E-mail*:</label><br/>
    	<input name = 'email' value = '<?php echo $row['email'] ?>'/><br/><br/>
		<td>Sede*::</td>
		<td>
			<select name='sede_id'>"
            <?php
